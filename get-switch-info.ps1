...
Write-Host $output
    ...
    {  write-host $_ : is Online - Proceeding with Packet Capture
       Start-Job -ScriptBlock {$res = Capture-LLDPPacket -ComputerName $using:_ | Parse-LLDPPacket ;$res | Add-Member -NotePropertyName Computername -NotePropertyValue $using:_ ; $res} -InitializationScript $func 
    }
    else
    {
...
foreach($job in Get-Job){
   $result += Receive-Job -Name $job.Name -Keep
}
